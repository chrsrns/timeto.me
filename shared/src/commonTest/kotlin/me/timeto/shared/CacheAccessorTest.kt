...
class CacheAccessorTest {
    ...
    fun activityOrNull_hitAndMiss() {
        Cache.activitiesDb = listOf(activity(1))
    ...
    fun indexRefreshesWhenListReplaced() {
        ...
        Cache.activitiesDb = listOf(activity(2))
    ...
    fun requireActivityByType_matchesType() {
        Cache.activitiesDb = listOf(activity(1), activity(2, type_id = ActivityDb.Type.other.id))
        assertEquals(2, Cache.requireActivityByType(ActivityDb.Type.other).id)
    }

    @Test
    fun activityDescendantsMap_includesDescendants() {
        Cache.activitiesDb = listOf(activity(1), activity(2, parent_id = 1), activity(3, parent_id = 2))
    ...
    fun taskFolderAccessors() {
        Cache.taskFoldersDbSorted = listOf(taskFolder(TaskFolderDb.ID_TODAY))
    ...
    fun noteFolderAccessors() {
        Cache.noteFoldersDb = listOf(noteFolder(5))
    ...
    @Test
    fun kvAccessors() {
        Cache.kvDb = listOf(KvDb(key = KvDb.KEY.ZEN_MODE_ENABLED.name, value = "0"))
